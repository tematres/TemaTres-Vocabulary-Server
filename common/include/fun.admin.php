    ...
    if (is_array($arrayVocab)) {
        switch ($do) {
            case 'A':
                ...
                $sql=SQLo(
                    "insert",
                    "into $DBCFG[DBprefix]term2tterm (tema_id,tvocab_id,tterm_url,tterm_uri,tterm_string,cuando,uid)
...
function admin_users($do, $user_id = "")
{
    ...
    switch ($do) {
        case 'actua':
            ...
            if ($arrayUserData["user_id"]==$userId) {
                $POSTarrayUser["status"]='ACTIVO';
            }

            $sql=SQL(
                "update",
                "$DBCFG[DBprefix]usuario
...
if ($_SESSION[$_SESSION["CFGURL"]]["ssuser_nivel"]=='1') {
	...
	($arrayTesa[titulo],$arrayTesa[autor],$arrayTesa[idioma],$arrayTesa[cobertura], $arrayTesa[url_base],now())"
            ...
            case 'M':
                ...
                $sql=SQL(
                    "update",
                    "$DBCFG[DBprefix]config SET titulo=$arrayTesa[titulo],
                    ...
                    if (strcasecmp($sendkey, $_SESSION['TGET_SEND_KEY'])===0) {
                        ...
                        */
                        if ($dataVocab->result->vocabulary_id=='1') {
                            ...
                            $sql=SQL(
                                "insert",
                                "into $DBCFG[DBprefix]tvocab (tvocab_label, tvocab_tag,tvocab_lang, tvocab_title, tvocab_url, tvocab_uri_service, tvocab_status, cuando, uid)
				VALUES
...
function setMetaTerm($term_id, $flag = 0)
{
    ...
    $sql=SQL(
        "update",
        "$DBCFG[DBprefix]tema set isMetaTerm='$flag', uid_final='$userId',cuando_final=now()
...
function SQLbulkReplaceTerm($from, $to, $where, $arrayTerms)
{
    ...
    return SQL(
        "update",
        "$DBCFG[DBprefix]tema  set tema=replace(tema, $from_string, $to_string),
						cuando_final=now(),
						uid_final=$userId
...
function SQLbulkReplaceNote($from, $to, $where, $arrayNotes)
{
    ...
    return SQL(
        "update",
        "$DBCFG[DBprefix]notas  set nota=replace(nota, $from_string, $to_string),
						cuando=now(),
						uid=$userId
...
function SQLfixDobleChar4Notes($notesType, $char, $charX2)
{
    ...
    return SQL(
        "update",
        "$DBCFG[DBprefix]notas set nota=replace(nota, '$charX2', '$char')
...
function SQLnoteshtml2chars()
{
    ...
    while ($array=$sql->FetchRow()) {
        ...
        $SQLupdate=SQL(
            "update",
            "$DBCFG[DBprefix]notas set nota='$nota'
						where id=$array[id]"
        );
    };
}
...
?>
>>>>>>> dee0fef384b565fb188728241563c33ae9b726c6
