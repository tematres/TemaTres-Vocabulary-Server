...
if($_SESSION[$_SESSION["CFGURL"]]["ssuser_nivel"]>0) {
    ...
    */
    function wichReport($task)
    {
        ...
        //All terms with her sources of authority
        case 'csv19':
            $sql=SQLreportAllSources4Terms();
            break;
...
function doArrayDatosTesauro($array)
{
    ...
    "id"=>doValue($array, "vocabulario_id")
    );
...
function do_r($id_mayor,$id_menor,$t_relacion,$rel_rel_id=0)
{
    ...
    if(($evalRecursividad == true) && ($okValues == true)) {
        ...
        $sql=SQL(
            "insert", "into $DBCFG[DBprefix]tabla_rel (id_mayor,id_menor,t_relacion,rel_rel_id,uid,cuando)
			values
...
function borra_r($id_r)
{
    ...
    case '2':
        $sql_id_delete=SQL(
            "select", "$DBCFG[DBprefix]tabla_rel.id
...
function addTerm($string,$tesauro_id,$estado_id=13)
{
    ...
    $sql=SQLo(
        "insert", "into $DBCFG[DBprefix]tema (tema,tesauro_id,uid,cuando,estado_id,cuando_estado,cuando_final,uid_final)
...
function abm_tema($do,$titu_tema,$tema_id="")
{
    ...
    case 'mod':
        ...
        $sql=SQLo(
            "update", "$DBCFG[DBprefix]tema set
...
function abm_target_tema($do,$tema_id,$tvocab_id,$tgetTerm_id,$tterm_id="0")
{
    ...
    if(is_array($arrayVocab)) {
        ...
        case 'A':
            ...
            $sql=SQLo(
                "insert", "into $DBCFG[DBprefix]term2tterm (tema_id,tvocab_id,tterm_url,tterm_uri,tterm_string,cuando,uid)
        ...
        case 'U'://update data
            ...
            $sql=SQLo(
                "update", "$DBCFG[DBprefix]term2tterm set
...
// 
function abmNota($do,$idTema,$tipoNota,$langNota,$nota,$src_id=0,$idNota="")
{
    ...
    case 'A':
        $sql=SQL(
            "insert", "into $DBCFG[DBprefix]notas
...
function abmURI($do,$tema_id,$array=array(),$uri_id=0)
{
    ...
    case 'A':
        ...
        if(strlen($parse_uri["scheme"])>1) {
            ...
            $sql=SQL(
                "insert", "into $DBCFG[DBprefix]uri
...
function admin_users($do,$user_id="")
{
    ...
    case 'alta':
        ...
        $sql=SQLo(
            "insert", "into $DBCFG[DBprefix]usuario
...
if($_SESSION[$_SESSION["CFGURL"]]["ssuser_nivel"]=='1') {
    ...
    function abm_vocabulario($do,$vocabulario_id="")
    {
        ...
        case 'A':
            ...
            $sql=SQL(
                "insert", "into $DBCFG[DBprefix]config (titulo,autor,idioma,cobertura,url_base,cuando)
	VALUES
	($arrayTesa[titulo],$arrayTesa[autor],$arrayTesa[idioma],$arrayTesa[cobertura], $arrayTesa[url_base],now())"
        ...
        case 'M':
            ...
            $sql=SQL(
                "update", "$DBCFG[DBprefix]config SET titulo=$arrayTesa[titulo],
            ...
            if($vocabulario_id=='1') {
                ...
                if(!$ctrl["value_id"]) {
                    ...
                    $sql1_6x1_7b=SQL(
                        "insert", "into `".$DBCFG[DBprefix]."values` (`value_type`, `value`, `value_order`, `value_code`) VALUES
                ...
                if(strcasecmp($sendkey, $_SESSION['TGET_SEND_KEY'])===0) {
                    ...
                    */
                    if($dataVocab->result->vocabulary_id=='1') {
                        ...
                        $sql=SQL(
                            "insert", "into $DBCFG[DBprefix]tvocab (tvocab_label, tvocab_tag,tvocab_lang, tvocab_title, tvocab_url, tvocab_uri_service, tvocab_status, cuando, uid)
				VALUES
        ...
        case 'M':
            ...
            */
            if($dataVocab->result->vocabulary_id=='1') {
                ...
                $sql=SQL(
                    "update", "$DBCFG[DBprefix]tvocab set
    ...
    function updateTemaTres($ver2ver)
    {
        ...
        switch ($ver2ver) {
        case '2_2x3_2':
            $task=SQLupdateTemaTresVersion('2_2x3_2');
            $rows=($task["2_2x3_2"]!==0) ? '<br/><span class="success">'.$install_message["306"].'</span>' : '<br/><span class="error">'.ERROR.'</span>'.print_r($task);
            break;
        case '1_6x1_7':
            ...
            if(is_array($params["includeNote"])) {
                ...
                while($arrayNotas=$sqlNotas->FetchRow()){
                    ...
                    if(($arrayNotas["tipo_nota"]!=='NP') && (in_array($arrayNotas["tipo_nota"], $params["includeNote"]))) {
                        $txt.='	'.$arrayNotas["label_tipo_nota"].': '.html2txt($arrayNotas["nota"]).' '.TXTsource4note($arrayNotas["nota_id"])."\r\n";
                    }
                };
            }
...
function do_mysql_dump($encode="utf8")
{
    ...
    $tables= $DBCFG["DBprefix"].'config,'.$DBCFG["DBprefix"].'tema,'.$DBCFG["DBprefix"].'tabla_rel,'.$DBCFG["DBprefix"].'indice,'.$DBCFG["DBprefix"].'usuario,'.$DBCFG["DBprefix"].'notas,'.$DBCFG["DBprefix"].'values,'.$DBCFG["DBprefix"].'tvocab,'.$DBCFG["DBprefix"].'term2tterm,'.$DBCFG["DBprefix"].'uri,'.$DBCFG["DBprefix"].'sources';
...
function abm_userNotes($do,$array,$value_id="0")
{
    ...
    case 'A':
        if(strlen($array["value"])>0) {
            ...
            $sql=SQL(
                "insert", "into $DBCFG[DBprefix]values
    ...
    case 'B':
        ...
        if((!in_array($value_id, array(8,9,10,11,15))) // not can be SYSTEM default notes
            ...
            $sql=SQL(
                "delete", " from $DBCFG[DBprefix]values
				where value_id='$value_id'
				and value_type='t_nota'"
            );
            $sql=array();
...
function HTMLformUserNotes()
{
    ...
    $rows.= '<th>'.ucfirst(LABEL_alias).':</th>';
    $rows.= '<th>'.ucfirst(LABEL_order).'</th>';
    $rows.= '<th></th>';
    $rows.= '</tr>';
    ...
    while ($array=$sql->FetchRow()){
        ...
        else
        {
            ...
            $rows.= '<td>'.$array["value_code"].'</td>';
            $rows.= '<td>'.$array["value_order"].'</td>';
            $rows.= ($array["cant"]>0) ? '<td>'.$array["cant"].' '.LABEL_notes.'</td>' : '<td><a onclick=preparaborrado2(\''.$array["value_id"].'\') title="'.ucfirst(LABEL_eliminar).'" href="#")><strong>'.ucfirst(LABEL_eliminar).'</strong></a></td>';
            $rows.= '</tr>';
        }
...
function HTMLformUserRelations()
{
    ...
    $t_relacion=secure_data($_GET["t_relation"], "int");
    $rows.='<tr>';
    $rows.='<th class="izq">';
    $rows.='<select id="t_relacion" name="t_relacion">';
    $rows.=doSelectForm(array("3#$LABEL_BT","4#$LABEL_UP","2#$LABEL_RT"), $t_relacion);
    $rows.='</select>';
    ...
    while ($array=$sql->FetchRow()){
        ...
        $rows.= ($array["cant"]>0) ? '<td>'.$array["cant"].'</td>' : '<td><a onclick=preparaborradoRel(\''.$array["rel_rel_id"].'\') title="'.ucfirst(LABEL_eliminar).'" href="#")><strong>'.ucfirst(LABEL_relationDelete).'</strong></a></td>';
        $rows.= '</tr>';
    }
...
function HTMLformURIdefinition()
{
    ...
    while ($array=$sql->FetchRow()){
        ...
        $rows.= ($array["uri_cant"]>0) ? '<td>'.$array["uri_cant"].'</td>' : '<td><a onclick=preparaborradoURI(\''.$array["uri_type_id"].'\') title="'.ucfirst(LABEL_eliminar).'" href="#moreURI")><strong>'.ucfirst(LABEL_URItypeDelete).'</strong></a></td>';
        $rows.= '</tr>';
    }
...
function setMetaTerm($term_id,$flag=0)
{
    ...
    $sql=SQL(
        "update", "$DBCFG[DBprefix]tema set isMetaTerm='$flag', uid_final='$userId',cuando_final=now()
...
function SQLbulkReplaceTerm($from,$to,$where,$arrayTerms)
{
    ...
    return SQL(
        "update", "$DBCFG[DBprefix]tema  set tema=replace(tema, $from_string, $to_string),
						cuando_final=now(),
						uid_final=$userId
...
function SQLbulkReplaceNote($from,$to,$where,$arrayNotes)
{
    ...
    return SQL(
        "update", "$DBCFG[DBprefix]notas  set nota=replace(nota, $from_string, $to_string),
						cuando=now(),
						uid=$userId
...
function SQLfixDobleChar4Notes($notesType,$char,$charX2)
{
        GLOBAL $DBCFG;
    return SQL(
        "update", "$DBCFG[DBprefix]notas set nota=replace(nota, '$charX2', '$char')
...
function SQLnoteshtml2chars()
{
    ...
    while($array=$sql->FetchRow()){
        ...
        $SQLupdate=SQL(
            "update", "$DBCFG[DBprefix]notas set nota='$nota'
						where id=$array[id]"
        );
    };
...
function do_pdfAlpha($params=array())
{
    ...
    include_once T3_ABSPATH . 'common/fpdf/fpdf.php';
    include_once T3_ABSPATH . 'common/include/fun.pdf.php';
...
EOT;
    ...
    { // mantem o navegador acordado
        ...
        while($arrayRelaciones=$sqlRelaciones->FetchRow()){
            ...
            if($arrayRelaciones["t_relacion"]==3) {    // inclusao de termos genericos 550 $a
                if($arrayRelaciones["bt_tema"]) {
        ...
        while($arrayNotas=$sqlNotas->FetchRow()){
            ...
            if(in_array($arrayNotas["tipo_nota"], $includeNotes)) {
                ...
                */
                ...
                $txt .='  <subfield code="a">'.html2txt($arrayNotas["nota"]).TXTsource4note($arrayNotas["nota_id"]).'</subfield>' . "\n";
                $txt .=' </datafield>' . "\n";
            }
        };
...
// 
// ALTA, BAJA de fuentes normalizadas de bibliografía
// 
function abm_sources($do,$src_id=0,$array=array())
{
    ...
    $src_alias=XSSprevent($array["src_alias"]);
    $src_note=XSSprevent(html2txt($array["src_note"]));
    $src_url=XSSprevent(html2txt($array["src_url"]));
    $src_url= filter_var($src_url, FILTER_VALIDATE_URL) ? $src_url : '';


    $src_status= (in_array($array["src_status"], array("1","0"))) ? $array["src_status"] : 0;

    switch($do){
    case 'A':

        $sql=SQL(
            "insert", "into $DBCFG[DBprefix]sources
			(src_alias,src_note,src_url,src_status,user_id,create_date)
			values ('$src_alias','$src_note','$src_url',$src_status,$userId,now())"
        );
        break;

    case 'B':
        $src_id=secure_data($src_id, "int");
        $array= ARRAYsource($src_id);
        if($array["src_cant"]==0) {    
            $sql=SQL("delete", "from $DBCFG[DBprefix]sources where src_id='$src_id'");
        }else  {
            $sql=false;
        }
        break;
    ...
    case 'M':
        $src_id=secure_data($src_id, "int");
        $sql=SQL(
            "update", " $DBCFG[DBprefix]sources set
					src_alias='$src_alias',
					src_note='$src_note',
					src_url='$src_url',
