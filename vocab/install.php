...
if (!defined('T3_WEBPATH')) define('T3_WEBPATH', getURLbaseInstall().'../common/');
...
function SQLtematres($DBCFG, $DB, $arrayInstallData = array())
{
    ...
    if ($result1) {
        ...
        $result2 = $DB->Execute(
            "INSERT INTO `".$prefix."config` (
                ...
                1, $title, $author, $tematres_lang, 'Controlled vocabulary', '2', '$today', NULL, '$url'
            );"
        );
    }
    ...
    $result10 = $DB->Execute(
        ...
        (2, 't_relacion', 'Termino relacionado', NULL, 'TR'),
    ...
    if (is_object($result7)) {
        ...
        $admin_surname = ($arrayInstallData['s_name']) ? $DB->qstr(trim($arrayInstallData['s_name']), get_magic_quotes_gpc()) : "'admin surname'";
        ...
        $result8 = $DB->Execute(
            "INSERT INTO `".$prefix."usuario` (`APELLIDO`, `NOMBRES`, `uid`, `cuando`, `id`, `mail`, `pass`, `orga`, `nivel`, `estado`, `hasta`)
            VALUES ($admin_surname,$admin_name, 1, now(), 1, $admin_mail,$admin_pass_hash, 'TemaTres', 1, 'ACTIVO', now())"
        );
...
function HTMLformInstall($lang_install)
{
    ...
    foreach ($CFG["ISO639-1"] as $langs) {
        ...
        $rows='
        <form class="form-horizontal" id="formulaire" name="formulaire" method="post" action="install.php">
            <fieldset>
            <!-- Form Name -->
            ...
            <div class="header">
                <h1>
                    <a href="http://www.vocabularyserver.com/" title="TemaTres: vocabulary server">
                        <img src="<?php echo T3_WEBPATH;?>/images/tematres-logo.gif"  alt="TemaTres"/>
                    </a>
